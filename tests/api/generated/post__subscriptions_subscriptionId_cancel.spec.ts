...
test('POST /subscriptions/{subscriptionId}/cancel - Cancel subscription', async ({ request, baseURL }) => {
  if (!baseURL) throw new Error('baseURL is not set');

  const authHeader = process.env.AUTH_HEADER;
  const urlPath = `/subscriptions/${process.env["PATH_SUBSCRIPTIONID"] || "REPLACE_SUBSCRIPTIONID"}/cancel`;
