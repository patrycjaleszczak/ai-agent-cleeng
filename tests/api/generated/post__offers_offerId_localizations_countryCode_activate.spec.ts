...
test('POST /offers/{offerId}/localizations/{countryCode}/activate', async ({ request, baseURL }) => {
  if (!baseURL) throw new Error('baseURL is not set');

  const authHeader = process.env.AUTH_HEADER;
  const urlPath = `/offers/${process.env["PATH_OFFERID"] || "REPLACE_OFFERID"}/localizations/${process.env["PATH_COUNTRYCODE"] || "REPLACE_COUNTRYCODE"}/activate`;
