...
test('POST /subscriptions/{subscriptionId}/resume - Resumes subscription from scheduled pause', async ({ request, baseURL }) => {
  if (!baseURL) throw new Error('baseURL is not set');

  const authHeader = process.env.AUTH_HEADER;
  const urlPath = `/subscriptions/${process.env["PATH_SUBSCRIPTIONID"] || "REPLACE_SUBSCRIPTIONID"}/resume`;
