...
test('POST /offers/season-groups/{id}/seasons - Add a season to a season group', async ({ request, baseURL }) => {
  if (!baseURL) throw new Error('baseURL is not set');

  const authHeader = process.env.AUTH_HEADER;
  const urlPath = `/offers/season-groups/${process.env["PATH_ID"] || "REPLACE_ID"}/seasons`;
