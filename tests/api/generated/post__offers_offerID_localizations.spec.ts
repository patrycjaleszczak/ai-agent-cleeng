...
test('POST /offers/{offerID}/localizations - Create new localization', async ({ request, baseURL }) => {
  if (!baseURL) throw new Error('baseURL is not set');

  const authHeader = process.env.AUTH_HEADER;
  const urlPath = `/offers/${process.env["PATH_OFFERID"] || "REPLACE_OFFERID"}/localizations`;
